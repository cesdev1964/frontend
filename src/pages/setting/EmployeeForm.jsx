...
export default function EmployeeForm({ title, isEdit = false }) {
  ...
  const [openFlowDisplay,setOpenFlowDisplay] = useState(false);
  const {
    flowDropdown,
    getFlowDropdown,
    getFlowById,
    ...
    if (isEdit === true) {
      if (employeeById && employeeIsLoading === false) {
        ...
        if (deductionList.length > 0) {
          ...
          setOpenFlowDisplay(true);
        // if(flowById.approvalSteps){
        // }
                    ...
                    <div>
                      ...
                      <div className="row form-spacing g-2">
                        <div className="col-md-12 col-lg-6">
                          <label className="form-label">
                            สายอนุมัติ <span className="muted">(คลิกดูสายอนุมัติได้)</span>
                            <span style={{ color: "red" }}>*</span>
                          </label>
                          <SearchDropdown
                            data={flowDropdown}
