...
export default function Profile({ title, isAdmin = false }) {
  ...
  const fetchDataTable = useCallback(async () => {
    ...
    try {
      if (employeeId) {
        await getEmployeeByIdForProfile(employeeId);
  ...
  useEffect(() => {
    ...
    if (!employeeDataProfile) {
      ...
      setEmpData(employeeDataProfile);


  }, [employeeDataProfile, isAdmin, employeeId]);

  useEffect(() => {
    const fetchFlowData = async () => {
      if (employeeDataProfile.employee?.flowId) {
        await getFlowById(employeeDataProfile.employee?.flowId);
      }
    };
  ...
  }, [employeeDataProfile, getFlowById]);

  const avatarUrl = employeeDataProfile?.files?.[employeeDataProfile?.files.length - 1]
    ?.photoPath
    ? `${baseURL}${
        employeeDataProfile.files[employeeDataProfile?.files.length - 1].photoPath
      }`
        ...
        {!isLoading ? (
          <>
            <div className="announcement-box position-relative p-4 py-5">
              ...
              <WorkInformation
                contratorDropdown={contratorDropdown}
                empData={empData}
              ...
              <FlowInformation flowById={flowById} />
            </div>
          </>
        ) : (
          <LoadingSpin />
