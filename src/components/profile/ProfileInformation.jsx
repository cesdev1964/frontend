...
export default function ProfileInformation({ avatarUrl, empData, titleDropdow }) {
  return (
    <div className="d-flex align-items-start justify-content-center">
      <div className="d-flex">
        <div className="d-flex flex-column align-items-center">
          ...
          <p className="mt-3 text-danger">
            <strong>
              รหัสพนักงาน : {empData?.employee?.employeeCode ?? "ไม่ระบุ"}
            </strong>
          </p>
          <h6 className="my-2 text-center" style={{ fontSize: "1.3rem" }}>
            {titleDropdow.find((item) => item.value === empData.titleId)
              ?.label ?? "คุณ"}{" "}
      ...
      <IsEmployeeStatusBadgeReact status={empData?.employee?.status} />
    </div>
  );
}
