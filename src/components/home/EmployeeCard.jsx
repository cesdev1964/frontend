import { useRef, useState, useEffect } from "react";
import ImageComponent from "../Image";
import DetailItem from "./detailItem";
import { useAuth } from "../../auth/AuthContext";
import { useEmployee } from "../../hooks/employeeStore";
import cesMascot from "../../assets/CES-logo.png";
import { useTitltName } from "../../hooks/titleNameStore";
import { usePosition } from "../../hooks/positionStore";
import { useEmployeeType } from "../../hooks/employeeTypeStore";
import { useJob } from "../../hooks/jobStore";
import { Dropdown } from "bootstrap/dist/js/bootstrap.bundle.min";

export default function EmployeeCard() {
  // const avatarUrl = `data:image/jpeg;base64,`;
  const avatarUrl = `data:image/jpeg;base64,`;


  const { logout, authdata, loading } = useAuth();
  const { employeeById, getEmployeeById } = useEmployee();
  const [onClickAccordian, setOnClickAccordian] = useState(true);
  const { getTitleDropdown, titleDropdown } = useTitltName();
  const { positionDropdown, getPositionDropdown } = usePosition();
  const { jobDropdown, getJobDropdown } = useJob();
  const { employeeTypeDropdown, getEmployeeTypeDropdown } = useEmployeeType();

  const [empData, setEmpData] = useState({});
  useEffect(() => {
    const fetchHome = async () => {
      if (authdata) {
        try {
          if (authdata.publicEmployeeId) {
            await getEmployeeById(authdata.publicEmployeeId);
            await getTitleDropdown();
            await getEmployeeTypeDropdown();
            await getJobDropdown();
            await getPositionDropdown();
          }
          // เกิด error เมื่อ token หมดอายุแล้วไม่สามารถแสดงหน้า Home ได้
          //  console.log("auth data",authdata.publicEmployeeId);
        } catch (error) {
          alert("โหลด API ไม่สำเร็จ", error);
        }
      }
    };
    // console.log("data", authdata);
    fetchHome();
  }, [
    getEmployeeById,
    authdata,
    getTitleDropdown,
    getEmployeeTypeDropdown,
    getJobDropdown,
    getPositionDropdown,
  ]);
  const PropNameEnum = Object.freeze({
    TitleName: "คำนำหน้า",
    PositionName: "ตำแหน่ง",
    EmployeeTypeName: "ประเภทพนักงาน",
    JobName: "หน่วยงาน",
  });

  useEffect(() => {
    if (employeeById && titleDropdown) {
      setEmpData(employeeById.employee);
      // console.log("emp data", employeeById.employee);
    } else {
      setEmpData(authdata);
    }
  }, [employeeById, titleDropdown]);

  const handleChangeCheckbox = () => {
    setOnClickAccordian((prev) => !prev);
  };

  const getName = (id, properties) => {
    switch (properties) {
      case PropNameEnum.TitleName:
        const titleName = titleDropdown.find((item) => item.value === id);
        return titleName ? titleName.label : "ไม่ระบุ";
      case PropNameEnum.PrositionName:
        const positionName = positionDropdown.find((item) => item.value === id);
        return positionName ? positionName.label : "ไม่ระบุ";
      case PropNameEnum.EmployeeTypeName:
        const employeeTypeName = employeeTypeDropdown.find(
          (item) => item.value === id
        );
        return employeeTypeName ? employeeTypeName.label : "ไม่ระบุ";
      case PropNameEnum.JobName:
        const jobName = jobDropdown.find((item) => item.value === id);
        return jobName ? jobName.label : "ไม่ระบุ";
      default:
        break;
    }
    // if(properties === PropNameEnum.TitleName){

    // }
    // const titlename =  titleDropdown.find((item)=>item.value === titleId)
    // return titlename?titlename.label:"ไม่ระบุ"
  };

  return (
    <>
      <div className="accordion">
        <div className="accordion-item">
          <input
            id="accordion-trigger-2"
            className="accordion-trigger-input"
            type="checkbox"
            checked={onClickAccordian === true}
            onChange={handleChangeCheckbox}
          ></input>
          <label
            class="accordion-trigger accordion-label"
            for="accordion-trigger-2"
          >
            <strong>บริษัท ซีอีเอส จำกัด</strong>
          </label>
          <section className="accordion-animation-wrapper">
            <div className="accordion-animation">
              <div className="accordion-transform-wrapper">
                <div className="accordion-content">
                  <div>
                    {authdata.publicEmployeeId ? (
                      // employee
                      <div className="d-flex align-items-start justify-content-center">
                        <div className="d-flex">
                          <div className="d-flex flex-column align-items-center">
                            <ImageComponent
                              imageSRC={avatarUrl}
                              height="140px"
                              width="140px"
                              borderRadius="10px"
                              alt="profile-avatar"
                              objectfit="cover"
                            />
                            <p className="mt-3 text-danger">
                              <strong>
                                รหัสพนักงาน :{" "}
                                {empData?.employeeCode ?? "ไม่ระบุ"}
                              </strong>
                            </p>
                            <h5 className="my-2">
                              {getName(
                                empData?.titleId,
                                PropNameEnum.TitleName
                              )}{" "}
                              {empData?.firstname ?? "ไม่ระบุ"}{" "}
                              {empData?.lastname ?? "ไม่ระบุ"}
                            </h5>
                            <div className="row g-4 p-2 ">
                              <div className="col-6  col-custom">
                                <DetailItem
                                  icon="fa-solid fa-user-tie"
                                  title="ตำแหน่ง"
                                  value={
                                    positionDropdown.find(
                                      (item) =>
                                        item.value ===
                                        Number(empData?.employee?.positionId)
                                    )?.label ?? "ไม่พบข้อมูล"
                                  }
                                />
                              </div>
                              <div className="col-6  col-custom">
                                <DetailItem
                                  icon="fa-solid fa-users"
                                  title="หน่วยงาน"
                                  value={getName(
                                    empData?.jobId,
                                    PropNameEnum.JobName
                                  )}
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="badge-style badge-stillWork">
                          พนักงาน
                        </div>
                      </div>
                    ) : (
                      // user
                      <>
                        <div className="d-flex align-items-start justify-content-center">
                          <div className="d-flex ">
                            <div className="d-flex flex-column align-items-center">
                              <ImageComponent
                                imageSRC={cesMascot}
                                height="140px"
                                width="140px"
                                borderRadius="10px"
                                alt="profile-avatar"
                                objectfit="cover"
                              />
                              <p className="mt-3 text-danger">
                                <strong>ยินดีต้อนรับสู่ เว็บไซต์ฝากเบิก</strong>
                              </p>
                              <h5 className="my-2">
                                คุณ
                                {authdata.firstname} {authdata.lastname}
                              </h5>
                              {/* <div className="row g-4 p-2 ">
                            <div className="col-6  col-custom">
                              <DetailItem
                                icon="fa-solid fa-user-tie"
                                title="ตำแหน่ง"
                                value="xxxxxxxxxxxxx"
                              />
                            </div>
                            <div className="col-6  col-custom">
                              <DetailItem
                                icon="fa-solid fa-users"
                                title="หน่วยงาน"
                                value="xxxxxxxxxxxxx"
                              />
                            </div>
                          </div> */}
                            </div>
                          </div>
                          <div className="badge-style badge-stillWork">
                            ผู้ใช้งาน
                          </div>
                        </div>
                      </>
                    )}
                    {/* ส่วนที่แบ่งระหว่ง user employee กับ user ทั่วไป */}
                  </div>
                </div>
              </div>
            </div>
          </section>
        </div>
      </div>
    </>
  );
}
